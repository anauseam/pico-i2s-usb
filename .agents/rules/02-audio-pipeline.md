...
## R2.1 — Hot-path function inventory
...
## R2.2 — Hot path is non-blocking

The following are FORBIDDEN inside any function listed in R2.1:
...
## R2.3 — Diagnostic logging is gated

Every `printf` in a hot-path function MUST be wrapped in
...
per-microframe prints are forbidden, even under the debug guard.

One-shot prints during `*_init` functions are allowed and need no guard;
init runs once, before streaming.

## R2.4 — DMA ping-pong contract

The DMA layer in `src/dma_audio.c` follows this contract; it MUST be
preserved:
...
Changing buffer count, IRQ number, or removing the `__dmb()` barriers is a
violation.

## R2.5 — Single application-owned SPSC hand-off
...
`main.c` (consumer). Its contract is fully specified by R2.4 — the
`buffer_*_ready` flags, the channel re-arm sequence, and the `__dmb()`
barriers.

No additional application-level ring buffer or FIFO is permitted between
`dma_audio` and TinyUSB. The audio path is:

```text
dma_audio (R2.4) → main.c (conductor) → tud_audio_write → ep_in_ff
```
...
Plans that introduce a second application-side FIFO between the DMA and
TinyUSB violate this rule. The previous design did this and it introduced
a 32-bit-sample byte-alignment hazard that caused audible distortion; the
refactor that established this rule removed that hazard.

## R2.6 — Rate matching is implicit via `ep_in_ff` packet-size variation
...
3. Never attempting partial writes — partial writes split a single DMA
   buffer across two IN packets at non-sample-aligned boundaries (see
   R2.5 rationale).
...
(i.e. it requires an explicit feedback EP), the correct response is to
implement feedback-EP-for-IN in TinyUSB upstream (currently TinyUSB
supports feedback EPs for OUT streams only, per `audio_device.h:558`
gated by
`CFG_TUD_AUDIO_ENABLE_EP_OUT && CFG_TUD_AUDIO_ENABLE_FEEDBACK_EP`) and
contribute the change. Application-side workarounds are not permitted.

## R2.7 — Sample format is fixed by descriptors
